...
| `raw/_sources` | Primary PDFs and source-of-record documents | various | reference | pipeline only | 135 | 50.8 | Where a cited figure can be re-checked against the document it came from. |
...
| `raw/maus` | Maus et al. global mining land use | CC BY 4.0 | reference | 2 builders | 1 | 24.7 | Satellite-derived mine footprints; a physical cross-check on where mining is. |
...
| `raw/mrds` | USGS Mineral Resources Data System | US public domain | reference | 1 builder | 1 | 16.9 | Deposit records, site grain. |
...
| `raw/surveys` | National geological survey extracts | mixed public | reference | 1 builder | 1 | 3.5 | Country-specific reserves and production where a survey publishes better than the global compilations. |
...
| `raw/osm` | OpenStreetMap extracts | ODbL | reference | 1 builder | 1 | 1.5 | Infrastructure geometry (ports, rail) for logistics work. |
...
| `raw/jasansky` | Jasansky et al. mine-level dataset | CC BY 4.0 | reference | 1 builder | 2 | 0.7 | Asset-level mine production - different grain from the cube, but the best public route to a bottom-up check. |
...
| `raw/icmm` | ICMM member and site data | ICMM terms | reference | 2 builders | 2 | 0.4 | Industry-side context. |
...
| `raw/ipis` | IPIS artisanal mining site data (DRC) | CC BY-SA | reference | 1 builder | 1 | 0.2 | DIRECTLY relevant to the cobalt gap: BGS under-reports DRC precisely because artisanal output does not enter national returns. |
...
| `raw/wikidata` | Wikidata entity extracts | CC0 | reference | 1 builder | 1 | 0.2 | Entity reconciliation helper. |
...
| `raw/usgs_critmin` | USGS critical-minerals deposit map (PP1802) | US public domain | reference | 1 builder | 1 | 0.1 | Deposit points, no time dimension. Site-level grain, so not cube material. |
...
| `raw/au_ozmin` | Geoscience Australia OZMIN | CC BY 4.0 | reference | 1 builder | 1 | 0.0 | Australian deposits and resources - a strong reserves source if a reserves layer is built. |
