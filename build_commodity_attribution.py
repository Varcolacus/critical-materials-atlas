...
registers = {
    'n_mrds': len(mrds_pts), 'n_osm': len(osm_pts), 'n_pp': len(pp_pts), 'n_au': len(au_pts),
    'jasansky': _union_cov([_J]),
    'jasansky_mrds': _union_cov([_J, _M]),
    'jasansky_mrds_osm': _union_cov([_J, _M, _O]),
    'all_sources': _union_cov([_J, _M, _O, _P, _AU]),
...
<article style="max-width:1060px">
  ...
  <p class="muted" style="margin-top:0">The obvious question &mdash; would more mine registers help? &mdash; tested directly. We stack <b>five independent public sources</b> onto Jasansky and re-run the join: <b>USGS MRDS</b> (<span id="nmrds"></span> sites), <b>OpenStreetMap</b> (<span id="nosm"></span> commodity-tagged mines), the <b>USGS critical-minerals deposit set</b> (<span id="npp"></span> curated points) and a national cadastre, <b>Geoscience Australia&rsquo;s operating mines</b> (<span id="nau"></span>). Coverage triples &mdash; and because the sources are independent, where two of them label the <i>same</i> mine we can check whether they <b>agree</b>.</p>
...
fetch('out/commodity_attribution.json').then(r=>r.json()).then(S=>{
  ...
  if(R){
    ...
    const rows=[['Jasansky only (this page&rsquo;s baseline)',R.jasansky,false],
      ...
      ['+ USGS critical-minerals + Australia cadastre (all five)',R.all_sources,true]];
    ...
    document.getElementById('regnote').innerHTML='So the honest answer to &ldquo;would more registers help?&rdquo; is <b>yes, a lot &mdash; up to a wall</b>: five independent public sources push labelling from 17% to <b>'+a.attributed_pct+'%</b> and critical-material coverage from 4% to <b>'+a.critical_pct+'%</b>. Note the diminishing returns &mdash; the national cadastre (Australia) barely moves the total, because the global sources already carry its major mines; its value is a fifth cross-check, not new ground. And <b>~'+Math.round(100-a.attributed_pct)+'% of the footprint still can&rsquo;t be labelled</b> &mdash; the irreducible gap: artisanal and small-scale mines that appear in <i>no</i> register (much of the world&rsquo;s cobalt, gold, tantalum, tin), plus the by-product problem above.';
    document.getElementById('xcheck').innerHTML='<b style="color:#0e7c74">Cross-check &mdash; do the sources agree?</b> Among the four sources that label mines by their <i>primary commodity</i> (Jasansky, MRDS, OpenStreetMap, Australia), where two label the same mine they <b>agree '+R.agreement_pct+'%</b> of the time ('+f(R.n_agree)+' agree, '+f(R.n_disagree)+' disagree) &mdash; strong mutual corroboration, and <b>'+R.high_conf_share_of_critical+'% of the critical-labelled footprint is confirmed by two or more sources</b> (a high-confidence tier). The remaining source, USGS critical-minerals, adds coverage on a <i>different</i> basis &mdash; it tags each deposit by its critical mineral of interest (often a by-product), not the primary product &mdash; so it lifts the critical share but isn&rsquo;t counted in the primary-commodity agreement.';
  }
});
</script>
</body></html>'''
