...
EDGES = [
    ...
    ('cobalt', 'Cobalt', 'high-performance batteries', 'nickel', 'thermal stability falls as nickel rises',
     'shift', 'High-nickel NMC uses less cobalt but MORE nickel — you swap DR Congo (75%) for Indonesia (62%). The dependency moves; it does not leave.'),
    ('lithium', 'Lithium', 'stationary & entry EV batteries', 'sodium (Na-ion)', 'lower energy density, earlier commercial stage',
     'escape', 'Sodium is one of the most abundant elements on earth. Na-ion cannot match lithium on density, but for grid storage and cheap EVs it is a genuine exit, not a swap.'),
    ('magnets', 'Rare-earth magnets', 'EV traction & wind motors', 'no-magnet motor (Cu + electronics)', 'lower efficiency/torque density, more copper & control electronics',
     'escape', 'Induction and externally-excited motors use no rare earths at all (Renault, Tesla have shipped both). The cost is efficiency and copper, not a new chokepoint.'),
    ...
    ('gallium', 'Gallium', 'power & RF semiconductors', 'silicon', 'lower efficiency at high power / high frequency',
     'escape', 'Silicon is the mature default GaN/GaAs improved on. Falling back loses performance but exits a 99%-China by-product entirely — for many uses, viable.'),
    ...
    ('antimony', 'Antimony', 'flame retardants', 'carbon / organics', 'reformulation cost',
     'escape', 'Phosphorus- and mineral-based flame retardants replace antimony trioxide in many plastics, exiting a China-controlled, export-restricted metal for abundant chemistry.'),
]
