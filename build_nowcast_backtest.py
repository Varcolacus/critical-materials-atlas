...
<article>
  ...
  <div class="rule"><b>None beats naive persistence at naming the top exporter</b> &mdash; 84.4% is the ceiling. One (shrinking last year&rsquo;s shares 70% toward the 5-year mean) marginally improves the <i>share</i> error (4.83&nbsp;&rarr;&nbsp;4.66pp). That is exactly the Meese&ndash;Rogoff result, now on our own data: a series this persistent leaves little for a cleverer extrapolation to add. So the honest conclusion is not &ldquo;our forecast is clever&rdquo; but &ldquo;<b>the world is stable, and the only real lever is the reconciliation engine&rsquo;s use of current-year data</b>&rdquo; &mdash; which is the pre-registered test above. One candidate we have <i>not</i> tried, plausibly better in low-N: hierarchical/panel shrinkage that borrows across all 32 materials. <span style="color:var(--mut);font-size:.9em">(Reproducible: <code>build_nowcast_models.py</code>.)</span></div>
