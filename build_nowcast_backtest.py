...
<article>
  ...
  <h2 class="sec">Can a smarter model beat persistence? We tried six.</h2>
  ...
  <div class="rule"><b>No model beats naive persistence at naming the top exporter</b> &mdash; 84.4% is the ceiling, because the leader is usually too dominant for any smoothing to flip the winner. But we then tested the <b>proper method for this kind of data</b> &mdash; <i>compositional</i> (log-ratio) models from the Aitchison / state-space literature &mdash; and one <b>does beat persistence on the harder task, predicting the leader&rsquo;s share</b>: a centred-log-ratio shrinkage cuts the share error from <b>4.83 to 4.51pp</b> (~7% better). So the honest verdict is split: persistence is optimal for <i>who</i> leads; a compositional model is modestly better for <i>by how much</i>. Both confirm Meese&ndash;Rogoff &mdash; a series this persistent leaves little to add &mdash; and neither is the real lever, which is the reconciliation engine&rsquo;s current-year data (the pre-registered test above). Untested SOTA: <b>Bayesian Dirichlet ARMA</b> (needs Bayesian tooling), which the literature finds wins most in <i>balanced</i> competition (5&ndash;25% shares) &mdash; the opposite of our concentrated regime, so gains are uncertain. <span style="color:var(--mut);font-size:.9em">(Reproducible: <code>build_nowcast_models.py</code>; methods: Aitchison 1986; Snyder/Ord state-space; BDARMA.)</span></div>
