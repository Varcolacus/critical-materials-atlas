"""Evidence JSON for the rare-earths (full basket) chain pilot. Uniform schema. Public sources.
Broader than the magnet chain: separation gates the whole basket. Run: python record_rare_earths.py
"""
from __future__ import annotations
import json, os

HERE = os.path.dirname(os.path.abspath(__file__))
OUT = os.path.join(HERE, "out", "rare_earths_chain.json")
...
CHAIN = {
    ...
    "published": True,
    "related": [{"href": "../magnet-chain/magnet-chain", "label": "Rare-earth magnet chain"}, {"href": "../phosphate-food-chain/phosphate-food-chain", "label": "Phosphate / food chain"}, {"href": "../boron-chain/boron-chain", "label": "Boron chain"}],
    "accent": "#8a4a6a",
    "eyebrow": "Product-chain pilot · seventeen elements, one chokepoint",
    ...
    "sections": [
        {"h2": "1 · The chokepoint is separation, not mining", "panels": [
            ...
            {"kind": "text", "h3": "The light-heavy split matters",
             "text": "The 'light' rare earths (lanthanum, cerium, neodymium) are more abundant; the 'heavy' ones "
                     "(dysprosium, terbium, yttrium) are scarcer and their separation is even more concentrated in "
                     "China. So the parts of the basket that matter most for high-performance magnets and phosphors are "
                     "exactly the parts with the thinnest alternative supply — which is what China's 2025 export "
                     "licensing targeted.",
             "flag": "the heavies are the tightest"},
        ]},
        {"h2": "2 · One separation, many products", "panels": [
            {"kind": "cards", "h3": "What the basket becomes", "cards": [
                {"t": "Magnets", "d": "Nd, Pr, Dy, Tb → NdFeB permanent magnets — the largest and most strategic use (see the magnet chain)."},
                {"t": "Phosphors & polishing", "d": "Eu/Tb/Y give the colours in displays and LED lighting; cerium oxide polishes glass and chips."},
                {"t": "Catalysts", "d": "Lanthanum-based catalysts crack crude oil in refineries; cerium aids autocatalysts — the quiet industrial uses."},
            ]},
        ]},
        {"h2": "3 · Why diversification is slow", "panels": [
            {"kind": "text", "h3": "Rebuilding separation takes years",
             "text": "New mines are being opened outside China (Mountain Pass in the US, Mount Weld in Australia, plus "
                     "Myanmar feedstock), but building non-Chinese separation capacity — the actual chokepoint — is "
                     "slow, capital-heavy and environmentally fraught. It is a built capability, so it can be rebuilt; "
                     "but on a timescale of years, which is why rare-earth security remains a live concern across "
                     "magnets, electronics and defence.",
             "flag": "a built capability, rebuilt slowly"},
        ]},
    ],
    "trade_intro": "BACI carries rare-earth metals (280530) and compounds (284690), but a traded oxide's customs origin "
                   "is the separation plant, not the mine, and the individual elements are lumped. Read the shares "
                   "below as the traded rare-earth chemicals — where China's separation dominance shows more than any "
                   "mine figure.",
    "method": [
...
print("wrote", os.path.relpath(OUT, HERE), "- rare earths (basket), separation ~90pct China gates magnets/phosphors/catalysts")
