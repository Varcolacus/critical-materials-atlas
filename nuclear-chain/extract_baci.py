"""Extract broad nuclear fuel-chain trade context from local BACI archives."""
...
OUT = os.path.join(HERE, "out", "nuclear_trade.json")
BATCHES = [
...
GROUPS = {
    "natural_uranium": {"title": "Natural uranium and compounds", "codes": ["284410"], "boundary": "Material trade for all listed forms; not uranium mining output, inventories or conversion services."},
    "enriched_products": {"title": "Enriched uranium or plutonium products", "codes": ["284420"], "boundary": "The HS heading combines enriched uranium, plutonium and compounds; it does not isolate enrichment services."},
    "reactors": {"title": "Nuclear reactors", "codes": ["840110"], "boundary": "Rare, lumpy project shipments; customs value is not installed or operating capacity."},
    "reactor_parts": {"title": "Nuclear-reactor parts", "codes": ["840140"], "boundary": "A broad parts heading without component, material, quality or project-status detail."},
}
...
def extract():
    ...
    for group, rows in output["series"].items():
        print(group, f"${rows[0]['world_usd']/1e6:.1f}M -> ${rows[-1]['world_usd']/1e6:.1f}M")
