"""Extract wind-equipment and broad component trade from local BACI archives."""
...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "out", "wind_trade.json")
BATCHES = [
...
CODES = {
    "850231": {"title": "Wind-powered generating sets", "boundary": "Complete generating sets; excludes towers and many separately traded components."},
    "730820": {"title": "Iron or steel towers and lattice masts", "boundary": "Includes electricity and telecom towers; not wind-specific."},
    "841290": {"title": "Parts of engines and motors of heading 8412", "boundary": "Can include wind-motor parts and blades but is not wind-specific."},
    "850300": {"title": "Parts for electric motors and generators", "boundary": "Includes parts for many motor and generator applications; not wind-specific."},
}
...
def extract():
    ...
    years = sorted(bag)
    out = {"source": "CEPII BACI V202601, based on UN Comtrade", "years": years, "vintage": {str(y): vintage[y] for y in years}, "codes": CODES, "years_data": {}, "series": {}}
    for year in years:
