...
<p class="sub">Extra-EU import dependency across eight critical raw materials, corrected for the Rotterdam/Antwerp transit effect. Source: Eurostat Comext (public).</p>
<p class="intro"><b>Naive</b> rankings by importing member state measure where goods are customs-cleared, not where they come from - distorted by the NL/BE port effect. <b>Corrected</b> rankings treat the EU as one entity and rank by country of origin (for extra-EU flows the Comext partner field is the origin). <b>Across all eight materials the constant is that the naive view is wrong</b> - China is the true origin for six of them, while antimony traces to Tajikistan and silicon to Norway. The gap between the two panels is the whole point.</p>
', sections, '
