"""Extract fibre-optics trade context from local BACI archives.

Trade context only: customs codes cannot see preform capability, germanium content or who lays
a subsea system. Read shares as availability of the traded good, not the maker structure.
"""
...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "out", "fibre_trade.json")
BATCHES = [
...
CODES = {
    "900110": {"title": "Optical fibres, bundles and cables (bare fibre)", "boundary": "Bare optical fibre; excludes finished telecom/subsea cable systems and repeaters."},
    "854470": {"title": "Optical fibre cables (sheathed)", "boundary": "Made-up optical fibre cables; routes through integrators; not subsea-system builders."},
    "811292": {"title": "Germanium/gallium etc. (unwrought basket)", "boundary": "Shared 811292 basket — germanium (the fibre dopant) not separable; upstream context only."},
}
