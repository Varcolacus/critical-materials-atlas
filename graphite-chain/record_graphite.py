"""Evidence JSON for the graphite / battery-anode chain pilot. Uniform schema. Public sources.
Shared chainview renderer, per-figure confidence tags. Run: python record_graphite.py
"""
from __future__ import annotations
import json, os

HERE = os.path.dirname(os.path.abspath(__file__))
OUT = os.path.join(HERE, "out", "graphite_chain.json")
...
SRC = {
    "usgs_graphite": {"title": "USGS Mineral Commodity Summaries 2026 — Graphite (Natural)", "year": 2026, "url": "https://pubs.usgs.gov/periodicals/mcs2026/mcs2026-graphite.pdf"},
    ...
    "bgs_wms": {"title": "BGS, World Mineral Statistics — graphite mine production", "year": 2024, "url": "https://www2.bgs.ac.uk/mineralsuk/statistics/worldStatistics.html"},
...
CHAIN = {
    ...
    "history": {
        ...
        "series": [
            ...
            {"label": "Brazil", "points": hist_points("graphite", "BR")},
        ],
    },
    "hops": [
    ...
    "sections": [
        {"h2": "1 · The chokepoint is the anode step, not the flake", "panels": [
            ...
            {"kind": "text", "h3": "Synthetic doesn't escape it",
             "text": "Anodes also use synthetic graphite, made by baking petroleum coke at high temperature — energy-"
                     "intensive, and again led by China. So the two roads to an anode, natural and synthetic, both run "
                     "through the same country. There is no ready second source for the negative electrode the way "
                     "there is beginning to be for some cathode inputs.",
             "flag": "both feedstocks, one country"},
        ]},
        {"h2": "2 · The lever was pulled in 2023", "panels": [
        ...
        {"h2": "3 · Why it stays concentrated", "panels": [
            {"kind": "text", "h3": "Cost, scale and the qualification wall",
             "text": "Building spherical-graphite and synthetic-anode capacity outside China is under way (the US, "
                     "Europe, Mozambique-to-elsewhere), but it is slow: the process is capital- and energy-heavy, and a "
                     "new anode material must be qualified into cell designs over years before a carmaker will trust it. "
                     "The chokepoint is being rebuilt elsewhere — but on a battery-qualification timescale, not a "
                     "policy one.",
             "flag": "a built chokepoint, rebuildable slowly"},
        ]},
    ],
    "trade_intro": "BACI carries natural graphite (powder/flake and other) and artificial graphite, but customs lines "
                   "do not separate battery-anode-grade spherical graphite from ordinary flake or refractory graphite. "
...
print("wrote", os.path.relpath(OUT, HERE), "- graphite, China ~74pct mine but ~100pct anode; export-controlled 2023")
