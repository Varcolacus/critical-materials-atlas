...
CHAIN = {
    ...
    # CORRECTED 6 Sep 2026. This read "Transformers / diffuse / batch-built equipment, spread across suppliers".
    # The transformer stage really is diffuse - measured, HHI 0.073 across 182 exporters - but filing the CHAIN
    # under that stage hid the fact that its tightest node sits one step upstream, in the core steel, at HHI
    # 0.164 and rising. The equipment queue is a lead-time problem; the steel is a capability one, and only the
    # second is a chokepoint in the sense this map means. Prompted by a reader; measured in grid-chain/out/goes.json.
    "chokepoint": {"product": "Electricity grid", "stage": "Grain-oriented electrical steel (transformer cores)",
                   ...
                   "holder": "China · Japan", "share": "53% of exports (CHN 28.8 + JPN 24.3, 2022–24 mean)",
                   "control": "—", "conf": "measured"},
    "published": True,
    ...
    "deck": "Copper and aluminium are abundant and diversified, and the equipment queues are real: a large "
            "power transformer can take up to four years to procure, and the world must roughly double its grid "
            "by 2040. But one material inside that equipment is not diffuse at all. Grain-oriented electrical "
            "steel — the transformer core — is the most concentrated node we measure in this chain, more so than "
            "the transformers themselves.",
    "byline": "copper & aluminium (abundant) ≠ grain-oriented electrical steel (HHI 0.164, rising) ≠ transformers & cables (years-long queues) ≠ the network",
    "correction": "You cannot mine your way to a working grid — but nor is the material question closed. Copper and "
                  "aluminium are not the binding constraint; large power transformers take up to four years to order and "
                  "HVDC cables more than five, while ~1,500 GW of renewables sit in connection queues. The correction to "
                  "our own earlier framing: the core steel is TIGHTER than the equipment it goes into. Grain-oriented "
                  "electrical steel exports run at HHI 0.164 against 0.073 for transformers, and the gap has been widening "
                  "since 2019.",
    "stats": [
        ...
        {"v": "HHI 0.164", "l": "grain-oriented electrical steel exports — the tightest node in this chain, above the transformers it feeds (0.073)", "conf": "measured"},
    ],
    "hops": [
        {"n": "1 · Metals & steel", "t": "copper and aluminium abundant and diversified — grain-oriented electrical steel is neither"},
    ...
    "sections": [
        {"h2": "1 · The chokepoint is the equipment — and the one steel inside it", "panels": [
            ...
            {"kind": "text", "h3": "Why: electrical steel and skilled winding", "conf": "measured",
             "text": "Transformer cores need grain-oriented electrical steel, and building a transformer takes skilled "
                     "winding and testing that cannot be rushed. We had asserted the steel was made by \"a small number of mills\" "
                     "while the chain diagram filed it with copper and aluminium as abundant. Measured, it is neither "
                     "abundant nor diffuse: <b>HHI 0.164</b>, China 28.8% and Japan 24.3% — <b>53% from two countries</b>, "
                     "61% from three. The transformers built from it score 0.073, copper wire 0.066, high-voltage cable "
                     "0.060. The core input is more than twice as concentrated as the equipment.",
             "note": "Computed from CEPII BACI HS17, codes 722511 + 722611, exporter value shares, 3-year mean 2022–2024 — grid-chain/out/goes.json.",
             "flag": "the tightest node in the chain is a steel, not a metal"},
            {"kind": "text", "h3": "And it is tightening", "conf": "measured",
             "text": "GOES export concentration has risen every year but one since 2019: 0.111, 0.127, 0.122, 0.146, 0.167, <b>0.188</b> in 2024. "
                     "China went from 27.3% of exports in 2022 to 31.7% in 2024; Russia, at 8%, is sanctions-exposed "
                     "supply on top. A chain whose tightest node is getting tighter is a different planning problem from "
                     "one with a stable bottleneck.",
             "note": "Same series, annual. Trade is not capacity: export shares say who SELLS, not who can MAKE. A "
                     "producer consuming its own output at home is understated — China notably — so 0.164 is a FLOOR on "
                     "the real concentration, and the direction of that error is known.",
             "flag": "0.111 → 0.188 in six years"},
        ]},
        {"h2": "2 · The metals are abundant — the steel is the exception", "panels": [
            {"kind": "text", "h3": "Copper and aluminium are not the wall here", "conf": "measured",
             "text": "Grids run on copper and aluminium, both diversified and recyclable. Aluminium even substitutes for "
                     ...
                     "timeline story told in the copper chain — but for building the grid this decade, neither metal is "
                     "the binding constraint. This holds for the conductors and NOT for the core steel, which is the "
                     "section above: grouping the three together was our error, and it survived because nobody had "
                     "measured the third one.",
...
print("wrote", os.path.relpath(OUT, HERE), "- grid, conductors abundant; bottleneck = transformer lead times + GOES core steel (HHI 0.164)")
