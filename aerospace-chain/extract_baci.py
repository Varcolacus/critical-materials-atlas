"""Extract aerospace-relevant trade context from local BACI archives.

This is TRADE CONTEXT, not a production series. Customs codes for engines and titanium do not
isolate aerospace-qualified material or single-crystal castings; read the shares as availability
of the traded form, not as factory or capability shares. Rhenium has no clean HS6 line and is
therefore handled from USGS production in record_aerospace.py, not here.
"""
...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "out", "aerospace_trade.json")
BATCHES = [
...
CODES = {
    "810820": {"title": "Titanium, unwrought; powders", "boundary": "Includes sponge and ingot; does NOT distinguish aerospace-qualified from commercial grade."},
    "750210": {"title": "Nickel, unwrought, not alloyed", "boundary": "Superalloy base metal, but most nickel trade is non-aerospace; context only."},
    "841112": {"title": "Turbojets of a thrust exceeding 25 kN", "boundary": "Large jet engines as traded; excludes engines fitted to delivered aircraft and captive OEM transfers."},
    "841122": {"title": "Turbo-propellers of a power exceeding 1,100 kW", "boundary": "Turboprop engines as traded; same captive-transfer caveat."},
    "841191": {"title": "Parts of turbojets or turbo-propellers", "boundary": "Engine parts as traded; a re-export/repair hub can surface as a 'maker'."},
}
