...
It's a set of static HTML pages (vanilla JS, no framework) generated by a Python build pipeline and
served from GitHub Pages, plus published JSON open-data in `out/`. No backend.

**Headline finding → [`FINDINGS.md`](FINDINGS.md):** across 32 critical materials, the top *exporter* is
not the top *miner* in **17 of 32** of them — import-origin statistics systematically overstate how
